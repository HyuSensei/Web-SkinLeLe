...
$sql_so_san_pham="select count(*) from san_pham where danh_muc=N'Quy Trình Dưỡng Da'";
...
if (isset($_POST['start']) && isset($_POST['end'])) {
    $start = $_POST['start'];
    $end = $_POST['end'];
    $sql = "select * from san_pham where danh_muc=N'Quy Trình Dưỡng Da' and gia >=$start and gia<=$end ";
} else {
    $sql = "select * from san_pham where danh_muc=N'Quy Trình Dưỡng Da' limit $so_san_pham_mot_trang offset $bo_qua";
    // $result = mysqli_query($connect, $sql);
}
$result = mysqli_query($connect, $sql);
?>
                ...
                <a href="product-details.php?id=<?php echo $value['id'] ?>">
                    ...
                    <div class="button-head">
                        <div style="width: 100%; height: 48px;" class="product-action-2">
                            <a style="width: 100%;" href="component/add_to_cart_csd.php?id=<?php echo $value['id'] ?>"> <button style="background-color: #8f212b;width: 100%;" type="button" class="btn btn-primary"><i class="fa-solid fa-cart-shopping"></i> Thêm vào giỏ hàng</button></a>
                        </div>
                    </div>
